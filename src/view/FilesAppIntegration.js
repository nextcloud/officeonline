...
export default {
	...
	initAfterReady() {
		...
		const officeonlineHeader = document.createElement('div')
		officeonlineHeader.id = 'officeonline-header'
		headerRight.insertBefore(officeonlineHeader, headerRight.firstChild)
	...
	insertGraphic(callback) {
		...
		OC.dialogs.filepicker(t('officeonline', 'Insert from {name}', { name: OC.theme.name }), function(path, type) {
			if (type === OC.dialogs.FILEPICKER_TYPE_CHOOSE) {
				const filename = path.substring(path.lastIndexOf('/') + 1)
				$.ajax({
					type: 'POST',
					url: OC.generateUrl('apps/officeonline/assets'),
					data: {
						path: path
					}
				}).done(function(resp) {
	...
	_addAvatarList() {
		...
		const avatarList = $('<div id="officeonline-avatars">')
		avatarList.on('click', function(e) {
			e.stopPropagation()
			$('#editors-menu').toggle()
		})
		$('#officeonline-header').append(avatarList)
	},

	_addHeaderShareButton() {
		if ($('header').length) {
			var $button = $('<div id="officeonline-sharing"><a class="icon-shared icon-white"></a></div>')
			$('#officeonline-header').append($button)
			$button.on('click', () => {
	...
	_addHeaderFileActions() {
		...
		OC.unregisterMenu($('#officeonline-actions .icon-more'), $('#officeonline-actions-menu'))
		$('#officeonline-actions').remove()
		var actionsContainer = $('<div id="officeonline-actions"><div class="icon-more icon-white"></div><ul id="officeonline-actions-menu" class="popovermenu"></ul></div>')
		var actions = actionsContainer.find('#officeonline-actions-menu').empty()
		...
		$('#officeonline-header').append(actionsContainer)
		OC.registerMenu($('#officeonline-actions .icon-more'), $('#officeonline-actions-menu'), false, true)
	},
	...
	_userEntry: function(view) {
		...
		if (view.ReadOnly === '1') {
			label.text(view.UserName + ' ' + t('officeonline', '(read only)'))
	...
	_avatarForView: function(view) {
		...
		var avatarContainer = $('<div class="officeonline-avatar"><div class="avatar" title="' + view.UserName + '" data-user="' + userId + '"></div></div>')
		...
		if (view.ReadOnly === '1') {
			...
			$(avatar).attr('title', view.UserName + ' ' + t('officeonline', '(read only)'))
		} else {
	...
	renderAvatars: function() {
		var avatardiv = $('#header .header-right #officeonline-avatars')
		avatardiv.empty()
		...
		for (var viewId in this.views) {
			...
			view.UserName = view.UserName !== '' ? view.UserName : t('officeonline', 'Guest')
		...
		var followCurrentEditor = $('<li><input type="checkbox" class="checkbox" /><label class="label">' + t('officeonline', 'Follow current editor') + '</label></li>')
	...
	addCurrentVersion() {
		if (this.getFileModel()) {
			...
			$('#versionsTabView').prepend('<ul id="lastSavedVersion"><li data-revision="0"><div><div class="preview-container"><img src="' + preview + '" width="44" /></div><div class="version-container">\n'
				+ '<div><a class="downloadVersion">' + t('officeonline', 'Last saved version') + '<span class="versiondate has-tooltip live-relative-timestamp" data-timestamp="' + mtime + '"></span></div></div></li></ul>')
			$('#versionsTabView').prepend('<ul id="currentVersion"><li data-revision="" class="active"><div><div class="preview-container"><img src="' + preview + '" width="44" /></div><div class="version-container">\n'
				+ '<div><a class="downloadVersion">' + t('officeonline', 'Current version') + '</a></div></div></li></ul>')
	...
	_restoreSuccess: function(response) {
		if (response.status === 'error') {
			OC.Notification.showTemporary(t('officeonline', 'Failed to revert the document to older version'))
		}
		...
		document.getElementById('officeonlineframe').src = document.getElementById('officeonlineframe').src
		OC.Apps.hideAppSidebar()
	},
	 ...
	 * the parameters officeonline_create and officeonline_filename are
	 * parsed by viewer.js and open a template picker in the new tab
	 */
	createNewFile: function(type) {
		OC.dialogs.prompt(
			t('officeonline', 'Please enter the filename for the new document'),
			...
			function(result, value) {
				if (result === true && value) {
					...
					var url = OC.generateUrl('/apps/files/?dir=' + dir + '&officeonline_create=' + type + '&officeonline_filename=' + encodeURI(value))
			...
			$buttons.eq(1).text(t('officeonline', 'Create a new document'))
		})
	}
}
